...
export const updateStatusById = async (req, res) => {
  ...
  try {
    ...
    if (event.type === 'dzikiran') {
      // masukan absen hadir ke member yang belum aktif, absen kematian >= 3 kali atau absen dzikiran >= 3 kali
      ...
      // untuk yang hadir dalam keadaan absen dzikiran kurang 3 kali maka reset absent_dzikiran
      await Member.updateMany(
        {
          _id: { $in: attendance_ids },
          $or: [{ $where: 'this.absent_dzikiran.length<3' }],
        },
        {
          $set: { absent_dzikiran: [] },
        }
      ...
      // untuk yang tidak hadir dalam keadaan status belum aktif, absen kematian lebih 3 kali atau absen dzikiran lebih 3 kali maka reset attandance_dzikiran
      await Member.updateMany(
        {
      ...
      // masukan absen ke member yang aktif, absen kematian dibawah 3 kali atau absen dzikiran dibawah 3 kali
      await Member.updateMany(
        {
          _id: { $nin: attendance_ids },
          status: 1,
