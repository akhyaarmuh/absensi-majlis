...
import { createPresent } from '../../fetchers/present-book';

const breadList = [{ title: 'Beranda', href: '/' },{ title: 'Kegiatan', href: '/event' }, { title: 'Absen hadir' }];
...
const Create = () => {
  ...
  const changeNoinduk = async (e) => {
    if(e.key === 'Enter'){
      try{
        const member = await createPresent({
          no_induk:e.target.value,
          event:event._id,
          type:event.type
        })
        setMember(member)
  ...
  return (
    <>
      ...
      <h3 className="text-lg">
        Buku hadir {
          event.type === 'kematian' ? `kifayah ${event.name}`:`dzikiran di ${event.name}`
        }
      </h3>

      <input
        className="w-full my-5 tablet:w-[300px] appearance-none rounded border py-2 px-3 text-sm leading-tight text-gray-700 shadow focus:outline-none dark:border-[#4B5563] dark:bg-charcoal dark:text-white"
        placeholder="Masukkan no. induk..."
        name="noInduk"
        autoComplete="off"
        autoFocus
        onKeyUp={changeNoinduk}
      />

      {member.full_name && (
        <div className="flex flex-col overflow-hidden rounded-lg bg-white shadow-lg dark:bg-neutral-700 tablet:max-w-xl tablet:flex-row">
          <img
            className="h-96 w-full self-center object-cover tablet:h-auto tablet:w-48"
